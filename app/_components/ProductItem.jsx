...
function ProductItem({ product }) {
    return (
        ...
        hover:scale-110 hover-shadow-lg transition-all ease-in-out cursor-pointer">
            <Image src={process.env.NEXT_PUBLIC_BACKEND_BASE_URL + product?.attributes?.images?.data?.[0]?.attributes?.url} alt={product?.attributes?.name} width={500} height={200}
            ...
            <Dialog>
                <DialogTrigger asChild>
                     <Button variant="outline" className="text-red-400 hover:text-white hover:bg-primary">Add to cart</Button>
                </DialogTrigger>
                <DialogContent>
                    <DialogHeader>
                    <DialogDescription>
